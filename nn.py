...
def CreateTestinScenario(name, train, test, architecture, interpolate=0, applyWindTransformation=0):
    ...
    np.savetxt(os.path.dirname(os.path.realpath(__file__)) + "/" + "".join(('data'+ str(architecture).replace(',', '-') + '-' + test + '-' + str(interpolate) + str(applyWindTransformation) +'.csv').split()), errorsTemperature, delimiter=',')
